...
export async function generateMetadata({ params }: Props) {
  ...
  if (!product) {
    return {
      title: "Product Not Found | Nexa Design Lab",
    };
  }

  const p = product as any;
  const title = p.seo_title || `${p.name} | Nexa Design Lab`;
  const desc = p.meta_description || p.short_hook ||
    `Premium ${p.category || "visual art"} designed for collectors by Nexa Design Lab.`;
      ...
      if (fullItems) {
        ...
        bundleItems = bundleItems.map((item: any) => {
          ...
          return {
            ...item,
            ...(fullItem || {}),
            // Ensure we use the cached name/price if not found, but prioritize live data
    ...
    if (related.length < 4) {
      ...
      if (relatedRaw) {
        relatedRaw.forEach((r: any) => {
          if (!existingIds.has(r.id) && r.id !== id) related.push(r);
        });
      }
    }
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 2,
        "name": p.name,
        "item": `https://nexadesignlab.com/products/${p.id}`
      }
    ]
  };
